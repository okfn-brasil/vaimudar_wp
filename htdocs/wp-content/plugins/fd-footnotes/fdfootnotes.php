...
function fdfootnote_convert($content) {
	...
	if (preg_match_all('/\[(\d+\. .*?)\]/s', $content, $matches)) {
		foreach($matches[0] as $fn) {
			...
			$n_desc = strip_tags($note);
			$content = str_replace($fn, "<sup class='footnote'><a href='$singleurl#fn-$post_id-$n' id='fnref-$post_id-$n' title='$n_desc' onclick='return fdfootnote_show($post_id)'>$n</a></sup>", $content);
			$n++;
		}
